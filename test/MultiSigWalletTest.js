...
describe('MultiSgiWallet deploy', function () {
    ...
    it('Should initialize correctly', async function () {
        ...
        const msw = await multiSigWalletFactory.deploy(
            owners,
            approvals
        );
        await msw.deployed();
    ...
    it('Should fail for invalid required approvals', async function () {
        ...
        let owners = accounts
            .slice(0, owner_count)
            .map((a) => a.address);
        await expect(multiSigWalletFactory.deploy(owners, approvals))
            .to.be.reverted;
    });
});
...
describe('MultiSgiWallet 2 out of 3', function () {
    ...
    it('Should execute a transfer when have enough approvals', async function () {
        ...
        await expect(msw.execute(txId))
            .to.emit(msw, 'Execute')
            .withArgs(txId);
    });

    it('Should execute a contract call when have enough approvals', async function () {
        testContractFactory = await ethers.getContractFactory(
            'TestContract'
        );
        testContract = await testContractFactory.deploy();
        await testContract.deployed();
        console.log(
            'deployed test contract at: ',
